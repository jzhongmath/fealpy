...
class DLDMicrofluidicChipModeler3D:
    """
    ...
    def get_options() -> Dict[str, Any]:
        ...
        return {
            ...
            'inlet_length': 1.0,
            'outlet_length': 1.0,
            'thickness': 1.0,  # thickness in z-direction
    ...
    def build(self, gmsh: gmsh):
        ...
        for (x, y, radius) in self.circles:
            cylinder = gmsh.model.occ.addCylinder(x, y, 0, 0, 0, thickness, radius)
            cylinder_tags.append(cylinder)
        
        gmsh.model.occ.cut(
            [(3, boundary_tag)], 
            [(3, tag) for tag in cylinder_tags], 
            removeObject=True, 
            removeTool=True
        )
        gmsh.model.occ.synchronize()
